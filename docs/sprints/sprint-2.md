...
**Hôte EVE-NG :** 192.168.140.160  
**Fichier du lab :** `/opt/unetlab/labs/PFE/Lab-PFE.unl`  
**UUID du lab :** `2e718c0b-41bf-48c0-a67a-8fac5dbb9e97`  
**Version EVE-NG :** 6.2.0
...
- Le nom du dossier image doit suivre le pattern **template_name-suffix** (ex : ubuntu-s1-1.0-pfe)
...
qemu_version: 2.4.0
qemu_options: -machine type=pc,accel=kvm -vga std -usbdevice tablet -boot order=c -cpu host
EOF
...
```
@reboot iptables -t nat -A POSTROUTING -o pnet0 -j MASQUERADE && echo 1 > /proc/sys/net/ipv4/ip_forward && iptables -I FORWARD -i pnet0 -j ACCEPT && iptables -I FORWARD -o pnet0 -j ACCEPT
```
...
|----------|-------|------------|
| Nœud grisé dans WebUI | qemu_version manquant dans YAML | Ajouter qemu_version: 2.4.0 au template |
| Boucle de boot PXE | -boot order=cd dans qemu_options | Changer en -boot order=c |
...
![Topologie EVE-NG — ISP-R1, SW1, pve1, S1, S2](./images/Pasted%20image%2020260522142958.png)

### 2. Templates de Nœuds Personnalisés — ubuntu-s1

![Template ubuntu-s1 dans la liste QEMU](./images/Pasted%20image%2020260522142750.png)

### 3. Templates de Nœuds Personnalisés — ubuntu-s2

![Template ubuntu-s2 dans la liste QEMU](./images/Pasted%20image%2020260522142800.png)

### 4. Dossier Image S1 sur l'Hôte EVE-NG

![Dossier ubuntu-s1-1.0-pfe avec virtioa.qcow2](./images/Pasted%20image%2020260522140137.png)

### 5. Dossier Image S2 sur l'Hôte EVE-NG

![Dossier ubuntu-s2-1.0-pfe avec virtioa.qcow2](./images/Pasted%20image%2020260522140228.png)

### 6. Configuration VLAN sur SW1

![show vlan brief — VLANs configurés](./images/Pasted%20image%2020260522134759.png)

### 7. Interfaces Trunk SW1

![show interfaces trunk — Gi0/0, Gi0/1, Gi0/2 en mode trunk](./images/Pasted%20image%2020260522134825.png)

### 8. Connectivité Internet ISP-R1

![show ip interface brief et ping 8.8.8.8](./images/Pasted%20image%2020260522134900.png)
```
